...
static const int gff_fid_mRNA=0;
static const int gff_fid_exon=1;
static const int gff_fid_CDS=2;
...
class GffLine {
 public:
    ...
    char* gseqname;
    char* track;
    char* ftype; //feature type: mRNA/gene/exon/CDS
    uint fstart;
    uint fend;
...
class GffExon : public GSeg {
 public:
  ...
  int qend;
  // byte flags; //bitmap of flags
  GffExon(int s=0, int e=0, double sc=0, char fr=0, int qs=0, int qe=0) {
    uptr=NULL;
    attrs=NULL;
    if (s<e) {
      start=s;
...
public:
  ...
  int addExon(uint segstart, uint segend, double sc=0, char fr='.',
             int qs=0, int qe=0, bool iscds=false, bool mergeCloseExons=false);

  int addExon(GffLine* gl, bool keepAttr=false, bool mergeCloseExons=false);
  ...
  GffObj(char* anid=NULL):exons(true,true,true) { //exons: sorted, free, unique
       gffID=NULL;
       uptr=NULL;
       ulink=NULL;
       udata=0;
   ...
   bool overlap(GffObj& d) {
    ...
    int exonOverlapLen(GffObj& m) {
      if (gstart>m.gend || m.gstart>gend) return 0;
      int i=0;
      int j=0;
      int ovlen=0;
   ...
   void xcoord(uint grstart, uint grend, char xstrand='+') {
     ...
     if (xstatus=='-') {
       exons.Reverse();
       int flen=gend-gstart;
       gstart=xend-gend+1;
       gend=gstart+flen;
       }
      else {
       gstart=gstart-xstart+1;
       gend=gend-xstart+1;
       }
     }
   ...
   bool operator==(GffObj& d){
       return (gseq_id==d.gseq_id && gstart==d.gstart && gend==d.gend && strcmp(gffID, d.gffID)==0);
       }
   bool operator>(GffObj& d){
      if (gseq_id!=d.gseq_id) return (gseq_id>d.gseq_id);
      if (gstart==d.gstart) {
         if (gend==d.gend) return strcmp(gffID, d.gffID)>0;
                      else return gend>d.gend;
         } else return (gstart>d.gstart);
      }
   bool operator<(GffObj& d){
     if (gseq_id!=d.gseq_id) return (gseq_id<d.gseq_id);
     if (gstart==d.gstart) {
...
class GffReader {
 ...
 protected:
  ...
  GHash<GffObj> phash; //transcript_id (Parent) => GffObj pointer
 public:
